...
use libp2p::{
    gossipsub::IdentTopic as Topic, identity::Keypair, multiaddr::Protocol,
    request_response::RequestResponseEvent, swarm::SwarmEvent, Multiaddr, PeerId,
};
...
fn get_store() -> UrsaStore<MemoryDB> {
...
fn insert_block(mut s: BitswapStorage<MemoryDB, DefaultParams>, b: &Block<DefaultParams>) {
    match s.insert(b) {
        Err(err) => error!(
...
async fn network_init(
    ...
    let mut service = UrsaService::new(keypair, config, store.clone())?;
...
async fn test_bitswap_get() -> Result<()> {
    ...
    let bitswap_store_1 = store_1.into();
    let mut bitswap_store_2: BitswapStorage<MemoryDB> = store_2.into();
...
async fn test_put_command() -> Result<()> {
    ...
    let (mut node_1, _node_1_addrs, peer_id_1, ..) = network_init(&mut config, None, None).await?;
