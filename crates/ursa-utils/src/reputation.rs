...
use crate::contract_bindings::reputation_score_bindings::{
    EpochScores, ReputationScoresCalls, SubmitScoresCall,
};
...
pub fn submit_scores_txn(scores: EpochScores, epoch: u64) -> Result<TransactionProto> {
    ...
    let data = ReputationScoresCalls::SubmitScores(SubmitScoresCall {
        epoch: epoch.into(),
        scores,
    })
    .encode();
